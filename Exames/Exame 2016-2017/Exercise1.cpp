/*
1. x(n+1) = x(n) + i --> A soma acumulada de números inteiros tem pouca suscetibilidade a erros de
...
está limitada. (definição = precisão)

2. x(n+1) = x(n) + h --> A soma acumulada de números em vírgula flutuante tem muita propensão a
...
3. x(n+1) = x0 + h*i --> Neste iterador são somados, à semelhança do iterados 2, números em vírgula
...
4. x(n+1)= xn +(1/2) ^m --> Este iterador é tão suscetível a erros como o 2. É efetuada uma soma
